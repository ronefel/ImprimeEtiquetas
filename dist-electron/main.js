...
process.env.VITE_PUBLIC = app$1.isPackaged ? process.env.DIST : path.join(process.env.DIST, "../public");
let win;
...
app$1.whenReady().then(() => {
  createWindow();
  ipcMain$1.handle("save-model", async (_, name, data) => {
    try {
  ...
  ipcMain$1.handle("get-models", async () => {
    return store.get("models", {});
  });
  ipcMain$1.handle("delete-model", async (_, name) => {
    const models = store.get("models", {});
    delete models[name];
    store.set("models", models);
    return { success: true };
