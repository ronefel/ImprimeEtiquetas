...
function App() {
    ...
    const handleUpdateLabel = (index, content) => {
        setLabelContents(prev => {
            const newContents = [...prev];
    ...
    const handleUpdateAlignment = (index, verticalAlign) => {
        setLabelContents(prev => {
            const newContents = [...prev];
    ...
    const handleLoadModel = (newConfig, newContents) => {
        // Compatibility with old save format (string[])
        let migratedContents;

        if (newContents.length > 0 && typeof newContents[0] === 'string') {
            migratedContents = newContents.map(c => ({ content: c, verticalAlign: 'center' }));
        } else {
            migratedContents = newContents;
        }
